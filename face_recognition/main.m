...
New_Feature_Learner = fullyConnectedLayer(Number_of_Classes,'Name',...
    'Model the appropriate layer','WeightLearnRateFactor', 10, 'BiasLearnRateFactor', 10);
New_Classifier_Layer = classificationLayer('Name', 'Last layer');
...
Image_Augmenter = imageDataAugmenter('RandXReflection', ...
    true,'RandXTranslation', Pixel_Range, 'RandYTranslation',...
    Pixel_Range,'RandXScale', Scale_Range, 'RandYScale', Scale_Range);
...
Training_Options = trainingOptions('sgdm','MiniBatchSize', Size_of_Minibatch, ...
    'MaxEpochs', 6,'InitialLearnRate', 3e-4,'Shuffle', 'every-epoch','ValidationData',...
    Augmented_Validation_Image,'ValidationFrequency', Validation_Frequency,'Verbose', false,'Plots', 'training-progress');
